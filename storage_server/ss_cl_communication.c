...
int handleWrite(int clientfd) {
    ...
    CHECK(send(clientfd,"ACK", 5, 0), -1, ERR_NETWORK_ERROR);
    for(int i = 2; i < count; i++) {
        if(priority[0]=='0')sleep(10);//for debugging purposes
        fprintf(f, "%s ", chunk_split[i]);
    }
    for(int i = 1; i < num_chunks; i++) {
        if(priority[0]=='0')sleep(10);
...
void* SSReceiveClientOperations(void *arg) {
    ...
    if(strcmp(operation_split[0], "READ") == 0) {
        ...
        printf("[DEBUG] Sending ACK to the receiver \n");
        CHECK(send(clientfd,"ACK", 5, 0), -1, ERR_NETWORK_ERROR);
        CHECK(send(storage_server_fd,"ACK", 5, 0), -1, ERR_NETWORK_ERROR);
